...
async function fetchNewsCategories(locale: string): Promise<NewsCategory[]> {
  try {
    const res = await fetch(
      `${API_BASE_URL}/categories?module_key=blog&is_active=1&locale=${locale}`,
      { next: { revalidate: 300 } },
    );
    if (!res.ok) return [];
...
export async function generateMetadata({
  ...
  const seo = await fetchSeoPage(locale, 'blog');
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '16px 16px 60px' }}>
            ...
            jsonld.collectionPage({
              ...
              mainEntity: jsonld.itemList(
                visiblePosts.slice(0, 12).map((item: any) => ({
                  name: item.title,
                  url: item.slug
                    ? localizedUrl(locale, `/blog/${item.slug}`)
                    : localizedUrl(locale, '/blog'),
                })),
              ),
            }),
          ])}
        ...
        {categories.length > 0 && (
          <div className="nw-chips">
            ...
            {categories.map((cat) => (
              <Link
                key={cat.id}
                href={localizedPath(locale, `/blog?category=${cat.slug}`)}
        ...
        <div className="nw-layout">
          ...
          <div>
            ...
            {featured && (
              <Link
                href={featured.slug ? localizedPath(locale, `/blog/${featured.slug}`) : '#'}
            ...
            {rest.map((post: any) => (
              <Link
                ...
                href={post.slug ? localizedPath(locale, `/blog/${post.slug}`) : '#'}
