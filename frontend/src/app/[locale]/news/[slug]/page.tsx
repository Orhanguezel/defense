...
export default async function NewsDetailPage({
  ...
  const shareUrl = `${SITE_URL}/${locale}/news/${slug}`;
  ...
  return (
    <>
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '16px 16px 60px' }}>
        ...
        {post.category_name && (
          <Link
            href={localizedPath(locale, `/news?category=${post.category_slug || ''}`)}
            style={{
          ...
          <div>
            ...
            >
              ...
              <RelatedLinks
                ...
                hrefBase={localizedPath(locale, '/news')}
            ...
            {relatedArticles.length > 0 && (
              <div className="nd-sidebar-card">
                ...
                {relatedArticles.map((ra: any) => (
                  <Link
                    key={ra.id ?? ra.title}
                    href={ra.slug ? localizedPath(locale, `/news/${ra.slug}`) : '#'}
                    className="nd-sidebar-item"
                  >
                    {resolveNewsImage(ra) && (
                      <div className="nd-sidebar-thumb">
