      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '16px 16px 60px' }}>
            ...
            jsonld.collectionPage({
              ...
              mainEntity: jsonld.itemList(
                visiblePosts.slice(0, 12).map((item: any) => ({
                  name: item.title,
                  url: item.slug
                    ? localizedUrl(locale, `/news/${item.slug}`)
                    : localizedUrl(locale, '/news'),
                })),
              ),
            }),
          ])}
        ...
        {categories.length > 0 && (
          <div className="nw-chips">
            ...
            {categories.map((cat) => (
              <Link
                key={cat.id}
                href={localizedPath(locale, `/news?category=${cat.slug}`)}
        ...
        {posts.length === 0 && (
          <>
            <SeoIssueBeacon
              type="soft-404"
              pathname={localizedPath(locale, '/news')}
        ...
        <div className="nw-layout">
          ...
          <div>
            ...
            {featured && (
              <Link
                href={featured.slug ? localizedPath(locale, `/news/${featured.slug}`) : '#'}
            ...
            {rest.map((post: any) => (
              <Link
                ...
                href={post.slug ? localizedPath(locale, `/news/${post.slug}`) : '#'}
            ...
            {sidebarPosts.length > 0 && (
              <div className="nw-sidebar-card">
                ...
                {sidebarPosts.map((sp: any) => (
                  <Link
                    key={sp.id ?? sp.title}
                    href={sp.slug ? localizedPath(locale, `/news/${sp.slug}`) : '#'}
